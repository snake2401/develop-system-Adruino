...
// NTC Sensor Init
    const int sensorPin = A1;          // Analog A1 pin
...
// Rotator servo settings
    ...
    int speed = 180;   // Change this to adjust the speed of the servo (0 to 180)
    int ease = speed; // Easying out the motorspeed to avoid suddenstops and reduce fricton and rotationforce on the mecanism
    int easestrength = 2;
...
// SETUP INTERFACE GRID
    ...
    tft.drawLine(0, 121, 320, 121, ST77XX_RED);
    tft.drawLine(159, 0, 159, 240, ST77XX_RED);
    tft.drawLine(160, 0, 160, 240, ST77XX_RED);
    tft.drawLine(161, 0, 161, 240, ST77XX_RED);
...
void loop() {
  ...
  if (lasttemp != temperature) {
    ...
    tft.fillRect(0,0,158,118, ST77XX_BLACK);
    tft.setCursor(30,40);
    if (temperature > 22){ tft.setTextColor(ST77XX_RED);};
    if (temperature == 22){ tft.setTextColor(ST77XX_GREEN);};
    if (temperature < 22){ tft.setTextColor(ST77XX_BLUE);};
